...
public class EmployeeInfoBetter {
	...
	HashMap<SortMethod, Comparator<Employee>> sortByHash = new HashMap<>();
	
	public EmployeeInfoBetter() {
		
		sortByHash.put(SortMethod.BYNAME, Comparator.comparing(byName).thenComparing(bySalary));
		sortByHash.put(SortMethod.BYSALARY, Comparator.comparing(bySalary).thenComparing(byName));
	}
